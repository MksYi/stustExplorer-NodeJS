...
var login = function(req, res, usr, pass, callback) {
	...
	request.get({
		url: 'https://flip.stust.edu.tw/service/loginagent/?account=' + usr + '&password=' + pass,
		jar: j,
	},function(error, response, body) {
		res.cookie( 'stustFlipCookie',
					j.getCookieString('https://flip.stust.edu.tw/'),
...
var Notice = function(req, res, usr, callback) {
	...
	request.get({
		url: 'https://flip.stust.edu.tw/user/' + usr + '/myCourse',
		encoding: null,
		jar: j,
		...
		/* Notice Start */
		request.get({
			url: 'https://flip.stust.edu.tw/user/' + usr + '/newest/doclist',
			encoding: null,
			jar: j,
			...
			for (var i = 0; i < Notice_hrefs.length; i++) {
				var elem = 			$("#docTbody_tr" + i);
				let itemArr = 		$(elem).find('td');
				var item = 			$(itemArr[0]).text().split(/\. |[\(]|[\)]/);
				var item_class = 	$(itemArr[1]).text().split("_");
				let num = 			item[0];
